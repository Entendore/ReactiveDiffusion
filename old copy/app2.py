...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QFormLayout, QPushButton, QSlider, QDoubleSpinBox, 
    QComboBox, QLabel, QSplitter, QStatusBar, QFileDialog,
    QSizePolicy, QCheckBox, QSpinBox, QFrame
)
from PySide6.QtCore import Qt, QThread, Signal, QTimer, QMutex, QMutexLocker
from PySide6.QtGui import QImage, QPixmap, QPalette, QColor, QMouseEvent

# =============================================================================
# PERFORMANCE BACKEND SETUP
# =============================================================================
...
def numba_step_buffered(U_src, V_src, U_dst, V_dst, Du, Dv, F, k, dt):
    """
    Optimized CPU kernel using double buffering (Ping-Pong).
    Prevents memory allocation inside the loop for massive speedup.
    """
    ...
    # Handle boundaries (copy edges) - simple approach
    # Numba doesn't allow slicing assignments in parallel mode easily, 
    # but boundary effects are negligible for this visualization.
...
class SimulationEngine:
    ...
    def _allocate_arrays(self):
        ...
        if self.use_gpu:
            self.U_src = cp.ones((self.height, self.width), dtype=cp.float32)
            self.V_src = cp.zeros((self.height, self.width), dtype=cp.float32)
            self.U_dst = cp.ones((self.height, self.width), dtype=cp.float32)
            self.V_dst = cp.zeros((self.height, self.width), dtype=cp.float32)
        else:
            # CPU (Numba)
            self.U_src = np.ones((self.height, self.width), dtype=np.float32)
            self.V_src = np.zeros((self.height, self.width), dtype=np.float32)
            self.U_dst = np.ones((self.height, self.width), dtype=np.float32)
            self.V_dst = np.zeros((self.height, self.width), dtype=np.float32)
    ...
    def reset(self, seed='multi_random'):
        ...
        if self.use_gpu:
            y, x = cp.meshgrid(cp.arange(self.height), cp.arange(self.width), indexing='ij')
        else:
            y, x = np.meshgrid(np.arange(self.height), np.arange(self.width), indexing='ij')

        # Seed Logic
        if seed == 'multi_random':
            for _ in range(15): # More blobs for immediate action
        ...
        # Sync buffers
        if self.use_gpu:
            self.U_dst = self.U_src.copy()
            self.V_dst = self.V_src.copy()
        else:
            np.copyto(self.U_dst, self.U_src)
            np.copyto(self.V_dst, self.V_src)
        ...
        with QMutexLocker(self.mutex):
            if self.use_gpu:
                ...
                for _ in range(num_blobs):
                    cx = int(cp.random.randint(0, self.width))
                    cy = int(cp.random.randint(0, self.height))
                    r = int(cp.random.randint(3, 10))
                    mask = (x - cx)**2 + (y - cy)**2 <= r**2
                    self.U_src[mask] = 0.5
                    self.V_src[mask] = cp.random.uniform(0.2, 0.4)
            else:
                y, x = np.meshgrid(np.arange(self.height), np.arange(self.width), indexing='ij')
                for _ in range(num_blobs):
                    cx = np.random.randint(0, self.width)
                    ...
                    mask = (x - cx)**2 + (y - cy)**2 <= r**2
                    self.U_src[mask] = 0.5
                    self.V_src[mask] = np.random.uniform(0.2, 0.4)

    def paint(self, x, y, radius=10, strength=0.5):
        with QMutexLocker(self.mutex):
            if self.use_gpu:
                y_coords, x_coords = cp.meshgrid(cp.arange(self.height), cp.arange(self.width), indexing='ij')
                dist = cp.sqrt((x_coords - x)**2 + (y_coords - y)**2)
                mask = dist < radius
                self.V_src[mask] = strength
                # Also update dst to prevent ghosting if paused
        ...
        while self.running:
            if not self.paused:
                ...
                if self.auto_disturbance:
                    disturbance_timer += 1
                    if disturbance_timer > 150: # Less frequent auto-disturbance
...
class FastImageViewer(QWidget):
    paint_signal = Signal(int, int, int, float) # x, y, radius, strength
    ...
    def _handle_mouse(self, event):
        ...
        # Scale to generic 512 coordinate system for signaling
...
class ControlPanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.check_auto = QCheckBox("Auto-Disturbance (Chaos)")
        ...
        self.spin_F.setToolTip("Feed Rate: How fast chemical U is replenished.")
        ...
        self.spin_k.setToolTip("Kill Rate: How fast chemical V is removed.")
        ...
        self.combo_map.addItems([
            ...
            "Jet", "Rainbow", "Hot", "Cool", "Ocean", "Twilight", "DeepGreen", "Bone"
        ])
        ...
        self.combo_preset.addItems(["Coral", "Mitosis", "Spirals", "Waves", "Spots"])
    ...
    def on_strength_change(self, val):
        pass # Slider handled directly in viewer signal usually, but we need to update viewer state.
        # Actually, we just read the slider value in the viewer loop or pass it.
        # Here we pass it to the viewer via the main window connection.
    ...
    def emit_params(self):
        ...
        d = {
            ...
            'colormap': maps.get(self.combo_map.currentText(), cv2.COLORMAP_INFERNO),
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("RD Simulator Pro MAX (Optimized)")
    ...
    def setup_ui(self):
        ...
        splitter.setSizes([280, 1020]) # Give more space to viewer
    ...
    def reset_sim(self):
        self.worker.paused = True
        seed = self.control_panel.combo_seed.currentText().lower()
